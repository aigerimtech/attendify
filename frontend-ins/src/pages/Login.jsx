...
const DEMO = { email: 'prof.smith@university.edu', password: 'instructor123' }
...
function ForgotPasswordModal({ onClose }) {
  ...
  async function delay() { await new Promise(r => setTimeout(r, 700)) }
  ...
  async function handleVerify(e) {
    e.preventDefault()
    if (otp !== DEMO_OTP) { setOtpError('Invalid code. Demo code: 123456'); return }
    setOtpError('')
    setLoading(true)
    await delay()
    setLoading(false)
...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    if (email === DEMO.email && password === DEMO.password) {
      login({ name: 'Prof. Smith', email, role: 'instructor', initials: 'PS' }, remember)
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12"
      ...
      <div className={`w-full max-w-sm rounded-2xl p-8 ${
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          {/* Email */}
          <div>
            <label className="block text-sm font-semibold mb-2" style={{ color: isDark ? '#e8e6f4' : '#1e1b4b' }}>Email</label>
            <div className="relative">
        ...
        <p className="text-sm text-center mt-6" style={{ color: 'var(--c-text-3)' }}>
          No account?{' '}
          <button onClick={() => navigate('/register')} className="font-bold text-primary-600 hover:text-primary-800 transition-colors">
            Register
          </button>
        </p>
      </div>
      ...
      <div className="mt-4 text-center">
        <p className="text-xs mb-2" style={{ color: 'var(--c-text-3)' }}>Demo credentials</p>
        <button onClick={fillDemo}
          className="text-xs font-semibold px-4 py-2 rounded-xl transition-all"
          style={{ background: 'var(--c-surface)', color: '#4338ca', border: '1px solid var(--c-border)', boxShadow: '0 1px 4px rgba(67,56,202,.08)' }}>
